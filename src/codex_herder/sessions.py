...
def build_bootstrap_message(
    ...
    return (
        ...
        f"{group_block}\n\n"
        "Startup behavior:\n"
        "- Do not make suggestions at startup of chat.\n"
        ...
        "- You may read elsewhere as needed, but only write inside the ITERATION FOLDER.\n"
        ...
        "- Every generated figure must have a same-base-name .md sidecar in the same folder, beginning with a brief summary followed by detailed reproducibility information and generation date/time.\n"
        "- Update the figure sidecar with a dated explanation whenever the figure changes.\n"
        ...
        "- Every generated video or video dataset must have a same-base-name .md sidecar in the same folder, beginning with a brief summary followed by detailed reproducibility information and generation date/time.\n"
        "- Update the video sidecar with a dated explanation whenever the video changes.\n"
        "- Store intermediate analysis stages that will be reused by subsequent analyses in output/processed_data/.\n"
        "- Store each named processed-data result in its own output/processed_data/<name>/ folder.\n"
        "- Every processed-data folder must contain description.md documenting exactly how its files were generated, including the date and time.\n"
        "- Update that description.md with a dated explanation whenever the processed data changes.\n"
        "- Code that refers to processed data must accept project_id, analysis_id, and iteration_id as parameters and resolve paths from them.\n"
        "- Do not hardcode absolute or project-specific paths in reusable analysis code.\n"
